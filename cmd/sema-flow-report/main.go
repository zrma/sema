...
func writeTextReport(writer io.Writer, report flow.MeasurementReport) error {
	lines := []string{
		...
		fmt.Sprintf("ingress samples_tickets=%d max_arrival_lag_ms=%d final_backlog_tickets=%d final_backlog_players=%d", report.Ingress.SamplesTickets, report.Ingress.MaxArrivalLagMillis, report.Ingress.FinalBacklogTickets, report.Ingress.FinalBacklogPlayers),
		...
		fmt.Sprintf("final idle=%d queued=%d ingress_tickets=%d ingress_players=%d in_game=%d cooldown=%d games=%d rating_min=%d rating_p10=%d rating_median=%d rating_p90=%d rating_max=%d rating_mean=%d rating_sd=%d", report.Final.IdlePlayers, report.Final.QueuedPlayers, report.Final.IngressBacklogTickets, report.Final.IngressBacklogPlayers, report.Final.InGamePlayers, report.Final.CooldownPlayers, report.Final.Rating.GamesPlayed, report.Final.Rating.Minimum, report.Final.Rating.Percentile10, report.Final.Rating.Median, report.Final.Rating.Percentile90, report.Final.Rating.Maximum, report.Final.Rating.Mean, report.Final.Rating.StdDev),
	}
