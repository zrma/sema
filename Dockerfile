...
RUN CGO_ENABLED=0 go build -trimpath -ldflags="-s -w -X main.version=${VERSION}" -o /out/sema-service ./cmd/sema-service && \
    ...
    CGO_ENABLED=0 go build -trimpath -ldflags="-s -w -X main.version=${VERSION}" -o /out/sema-conformance ./cmd/sema-conformance && \
    ...
    cp /out/sema-service /out/sema-server /out/sema-target-server /out/sema-conformance /out/sema-target-smoke /out/sema-postgres-migrate /out/sema-healthcheck /out/sema-ops-check /out/rootfs/usr/local/bin/ && \
