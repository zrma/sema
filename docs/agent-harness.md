...
Publication class는 원격 visibility가 결정되지 않은 greenfield 저장소이므로 `internal`에서 시작한다. public 전환은 repository gate, 권한 있는 machine-local private-inventory gate, live visibility 확인을 모두 통과한 별도 결정으로 다룬다.
...
대기 중인 플레이어·파티와 기존 세션의 빈자리를 대상으로 hard constraint를 만족하는 다양한 후보 조합을 탐색하고, 품질과 운영 비용을 함께 고려한 설명 가능한 match proposal과 확정 assignment를 생성한다.
...
- 무컨텍스트 시작점: `docs/HANDOFF.md`; 현재 작업 계약: 활성 `docs/todo-*/spec.md`와 `open-questions.md`.
...
2. discovery, constraint, scoring, proposal, reservation, assignment 중 이번 논리 경계를 고정한다.
...
제품 semantics, fairness와 latency의 우선순위, consistency guarantee, 구현 언어/runtime topology, credential·비용, 파괴적 변경, public 전환, published history rewrite, 승인되지 않은 push가 필요할 때만 사용자에게 최소 판단을 요청한다.
...
대표 new-match와 backfill fixture에서 완료성, proposal 품질, constraint 위반, 결정성, 설명 가능성, 탐색 latency, resource cost를 평가한다. 반복 실패는 domain test, benchmark, validation script, concise operating rule 중 가장 가까운 계층에 기계화한다.
...
- planner는 immutable snapshot을 받아 하나 이상의 `MatchProposal`을 만들며 외부 상태를 직접 변경하지 않는다.
- coordinator는 proposal을 reservation과 assignment로 전이시키며 모든 mutation을 idempotent하게 만든다.
...
- Active foundation: `docs/todo-0001-foundation/spec.md`, `docs/todo-0001-foundation/open-questions.md`.
