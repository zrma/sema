...
Sema는 public source repository다. 원격 visibility와 repository 선언은 항상 일치해야 하며 public push 전에는 repository gate와 권한 있는 machine-local private-inventory gate를 모두 통과한다. Go package는 아직 `internal/`에 머물러 public API compatibility를 약속하지 않는다.
