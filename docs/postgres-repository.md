...
이 adapter의 존재는 현재 V0 `cmd/sema-server` runtime cutover를 의미하지 않는다. 기존 journal/HTTP service는 authenticated target API와 explicit import/cutover가 준비될 때까지 reference surface로 남는다.
