...
- rating density는 기존 9개 `CenteredHistogram` bucket과 label 의미를 유지한다.
- chart 높이가 bucket 수보다 크면 각 bucket의 density cell을 가용 row에 비례해 반복해 panel 전체 높이를 채운다.
...
- chart 높이가 9줄보다 작으면 기존처럼 인접 histogram bucket을 묶어 축약한다.
...
- density intensity와 1500-centered histogram의 기존 관찰용 read-model 경계를 유지한다.

## Completion Evidence

- tall snapshot에서 rating density의 마지막 bucket row가 analytics panel 하단까지 배치된다.
