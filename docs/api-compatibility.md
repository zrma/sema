...
public import path는 `github.com/zrma/sema/alpha`, API marker는 `v0alpha1`이다. source와 example은 공개되어 있지만 production stability와 semantic-version compatibility를 아직 약속하지 않는다.
...
첫 breaking change가 생기면 다음 형식으로 문서를 추가한다.

```text
docs/migrations/v0alpha1-to-v0alpha2.md
```

문서는 변경된 symbol/field, behavior difference, before/after code와 rollback limitation을 포함한다. 현재는 migration 문서가 필요한 breaking alpha release가 없다.
