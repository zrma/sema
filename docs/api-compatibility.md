...
public import path는 `github.com/zrma/sema/alpha`, API marker는 `v0alpha2`다. source와 example은 공개되어 있지만 production stability와 semantic-version compatibility를 아직 약속하지 않는다.
...
P18 global batch objective 변경은 첫 breaking alpha change이며 다음 migration 문서가 소유한다.
