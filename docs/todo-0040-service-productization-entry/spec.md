...
- Status: In Progress — Storage Decision Gate Reached
...
adapter-neutral contract, migration inventory, persistent prototype과 crash/contention evidence가 완료되었다. file prototype은 semantic oracle이지 product storage가 아니며, 첫 target adapter와 write authority를 PostgreSQL primary로 채택할지는 사용자 결정을 기다린다. 승인 뒤 authenticated target API fixture와 실제 database adapter를 같은 contract 위에 구현한다.
...
database/topology와 stable API compatibility는 cost, external dependency 또는 irreversible migration을 동반하므로 fixture/benchmark/failure evidence와 사용자 결정을 요구한다. persistent evidence는 `docs/repository-adapter-evidence.md`까지 준비되었으므로 이제 database와 initial write authority 결정 전에 멈춘다.
