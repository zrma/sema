...
- Status: In Progress — PostgreSQL Adapter Complete
...
adapter-neutral contract, migration inventory, persistent prototype과 crash/contention evidence에 이어 PostgreSQL primary가 target write authority로 채택되었고 실제 adapter conformance도 완료되었다. file prototype은 semantic oracle로만 남고 Redis는 사용하지 않는다. 다음 slice는 authenticated target API schema, tenant authorization, pagination/polling fixture와 PostgreSQL-backed runtime composition 경계를 구현하는 것이다.
...
PostgreSQL primary와 no-Redis baseline은 ADR 0017로 승인되었다. stable API compatibility, authentication provider, production deployment/cutover와 numeric SLO는 별도 비용·호환성 경계이므로 fixture와 사용자 결정 없이 확정하지 않는다.
