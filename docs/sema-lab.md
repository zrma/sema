...
이 실행 파일은 안정적인 SDK나 production service가 아니다. built-in workload와 `v0alpha5` JSON은 평가 계약을 발견하기 위한 실험 표면이며 호환성을 아직 약속하지 않는다.
...
- candidate ticket window의 intentional quality-gap diagnostic.
- solo/duo/trio와 backfill을 함께 처리하는 small batch-frontier fixture.
- candidate graph 제한으로 global batch coverage gap을 만드는 batch-frontier diagnostic.
...
- eligible small snapshot의 exhaustive batch-frontier relation, search counters와 dominating witness.

`-details`는 proposal ID, kind와 team별 ticket placement를 추가한다. JSON report는 같은 aggregate와 모든 proposal placement/evidence를 `schema_version: v0alpha5` envelope에 담는다. report는 candidate ticket/window truncation과 exact placement candidate뿐 아니라 global batch candidate/selection 수, relative utility, generation/selection node와 truncation을 구분한다. 기존 oracle은 two-team/12-ticket 이하의 single new-match quality를 비교한다. 별도 batch frontier는 최대 12 match ticket, 2 backfill ticket, 2 team의 모든 admissible disjoint batch를 열거해 planner point, nondominated points와 witness를 기록한다.
