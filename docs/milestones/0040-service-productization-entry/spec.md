...
- `internal/repository/fileprototype`이 같은 conformance를 통과하고 실제 subprocess crash-before/after-commit recovery를 구분한다.
- `repositorytest.BenchmarkAdapter`가 memory/file adapter에 동일한 sequential 및 contended CAS workload를 적용한다.
- `docs/repository-adapter-evidence.md`가 prototype의 full-state rewrite와 single-process writer 한계 및 PostgreSQL 권장안을 기록한다.
- ADR 0017이 PostgreSQL primary authority와 stateless service replica를 채택하고 Redis를 baseline에서 제외한다.
- `internal/repository/postgres`가 repository-owned schema, explicit migration과 separate-pool transaction authority를 구현한다.
- `scripts/check-postgres.sh`가 실제 PostgreSQL에서 공통 conformance와 ordered multi-pool commit fixture를 실행한다.
- `internal/api/v0alpha2`, `internal/targetapi`와 `internal/service.MatchTickets`가 authenticated tenant scope, historical idempotency replay, active polling과 version-bound opaque pagination을 실행한다.
- PostgreSQL composition fixture가 같은 target handler에서 match ticket create와 poll을 실제 database까지 연결한다.
...
adapter-neutral contract, migration inventory, persistent prototype과 crash/contention evidence에 이어 PostgreSQL primary가 target write authority로 채택되었다. provider-neutral `v0alpha2` match-ticket vertical slice가 tenant authorization, historical idempotency, pagination/polling과 실제 PostgreSQL composition을 검증한다. file prototype은 semantic oracle로만 남고 Redis는 사용하지 않는다. 다음 milestone은 나머지 lifecycle command service와 identity provider/cutover 경계다.
...
PostgreSQL primary와 no-Redis baseline은 ADR 0017, provider-neutral target API boundary는 ADR 0018로 승인되었다. stable API compatibility, authentication provider, production deployment/cutover와 numeric SLO는 별도 비용·호환성 경계이므로 fixture와 사용자 결정 없이 확정하지 않는다.
