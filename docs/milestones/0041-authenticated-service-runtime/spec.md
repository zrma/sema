...
P29의 PostgreSQL authority와 authenticated match-ticket vertical slice를 proposal/reservation/assignment lifecycle까지 확장하고, V0 journal을 optional import/reference source로 유지하면서 authenticated runtime과 reference deployment acceptance를 만든다.
...
- `v0alpha1` journal/API는 별도 V0 development/reference 및 optional import source다.
...
- [x] 실제 deployment에서 provider-neutral identity mapping, private reachability와 E2E acceptance를 redacted evidence로 검증한다.
- [x] optional V0 import의 backup/restore 및 pre-writer recovery rehearsal을 검증한다.
...
- deployment acceptance와 import recovery/backup evidence가 redacted aggregate로 남는다.

## Out Of Scope

- stable v1 wire/SDK compatibility 선언.
- consumer evidence 없는 streaming, outbox, Redis와 cross-region multi-primary.
- representative workload 없이 임의의 numeric quota, SLO 또는 retention 확정.

## Decision Gate

provider-neutral lifecycle, optional V0 import, local PostgreSQL recovery rehearsal, OIDC/JWT validation, remote executable과 reference deployment acceptance가 완료되었다. 실제 provider, endpoint, credential과 private environment identity는 이 공개 저장소에 기록하지 않는다. 다음 P31은 PostgreSQL/OIDC runtime을 표준 surface로 승격하고 repository-owned compatibility, multi-replica, load/failure와 recovery evidence를 만든다.
