...
- `maxCandidateTickets`: proposal search에 전달할 oldest fitting ticket window. 0은 unbounded다.
- `maxCandidatesPerProposal`: unanchored/anchored candidate search 하나에서 비교할 exact placement 상한. 0은 구현 기본값을 사용한다.
- `maxBatchCandidates`: deduplicate 뒤 global selector에 전달할 proposal candidate 상한. 0은 구현 기본값을 사용한다.
- `maxBatchSearchNodes`: weighted set-packing branch-and-bound node 상한. 0은 구현 기본값을 사용한다.
...
- `evidence`: relaxation level, role penalty, team skill gap, oldest/total wait, maximum latency, candidate ticket window, 비교한 exact candidate/search budget과 batch-relative selection utility.
...
- `budgetExhausted`: candidate ticket, exact candidate 또는 node budget 때문에 best-known 결과로 종료했는지 나타낸다.
- `evidence`: candidate/selected proposal 수, selected backfill 수, wait-priority eligible/selected demand와 oldest waits, total rank utility와 generation/selection node 및 truncation.

각 unmatched 항목은 `hard_constraint`, `insufficient_capacity`, `quality_threshold`, `search_budget`, `proposal_limit`, `batch_objective` 중 stable 대표 reason을 가진다. 한 batch의 `MatchTicket`은 최대 한 proposal에만 나타난다. 하나의 `BackfillTicket`도 최대 한 backfill proposal에만 나타난다. planner는 같은 ticket 집합의 team 대안을 candidate generation에서 deduplicate하고 선택된 proposal만 반환한다.
