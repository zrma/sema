...
5. mutation command는 읽은 resource version을 CAS precondition으로 제출한다.
6. resource changes, operation receipt와 redacted audit receipt를 한 commit으로 기록한다.
7. 응답이 유실되면 같은 operation ID와 digest를 재시도해 최초 commit receipt를 읽는다.
...
| commit 전 process crash | operation receipt와 mutation 모두 없음 | 같은 command retry가 새 commit 수행 | persistent adapter crash fixture에서 검증 예정 |
| commit 후 response 전 process crash | receipt와 mutation 모두 존재 | 같은 ID/digest retry가 최초 result 반환 | repository reopen conformance, `TestRuntimeRecoversReservationAssignmentAndIdempotency` |
...
| cross-tenant resource 접근 | repository key/query 실행 전 거부 | 인증/인가 오류; resource existence를 누출하지 않음 | target auth adapter fixture에서 검증 예정 |

## API Resource Model

모든 target identity와 operation ID의 idempotency 범위는 tenant scope 안이다. resource key는 `(tenant scope, resource kind, resource ID)`다. transport version 이름은 persistent prototype과 consumer review 전까지 확정하지 않지만 다음 operation semantics는 adapter와 무관하게 유지한다.
