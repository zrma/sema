...
P0 architecture foundation부터 P5 executable lab과 P6 deterministic workload-evaluation baseline까지 완료되었다. 현재 구현은 deterministic bounded candidate ranking부터 terminal assignment acknowledgment까지 하나의 in-process facade로 실행 가능하고 built-in/synthetic workload에서 coverage, proposal evidence와 small-case oracle relation을 직접 확인할 수 있다. 아직 production-ready service나 안정적인 public SDK는 아니다.

## Public Contract

- Go module identity는 `github.com/zrma/sema`다.
- source는 Apache License 2.0으로 공개한다.
- 모든 Go package는 아직 `internal/`에 있으며 public API와 semantic-version compatibility를 약속하지 않는다.
- 첫 integration baseline은 same-process direct call, producer replay, synchronous acknowledgment와 single replica다.
