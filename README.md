...
P0 architecture foundation부터 P8 public alpha integration까지 완료되었고 P9 durable runtime을 진행 중이다. deterministic composition부터 terminal assignment acknowledgment까지 실행 가능하며 single-writer journal이 reservation, assignment와 decision audit을 재시작 뒤 복구한다. public `alpha` package로 side-effect-free composition을 import할 수 있지만 아직 production-ready service나 안정적인 SDK는 아니다.
