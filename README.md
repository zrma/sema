...
P0 architecture foundation부터 P9 versioned durable service와 P10 container operations까지 구현되었다. deterministic composition부터 terminal assignment acknowledgment까지 실행 가능하며 single-writer journal이 reservation, assignment와 decision audit을 재시작 뒤 복구한다. hardened local container baseline은 있지만 인증된 remote production deployment나 안정적인 SDK는 아직 아니다.
