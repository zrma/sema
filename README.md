...
P0 architecture foundation부터 P11 interactive flow visualization까지 구현되었다. deterministic composition부터 terminal assignment acknowledgment까지 실행 가능하며 single-writer journal이 reservation, assignment와 decision audit을 재시작 뒤 복구한다. reference container SLO는 통과하지만 인증된 remote production deployment나 안정적인 SDK는 아직 아니며 v1 release는 gate가 차단한다.
...
- `docs/sema-flow.md`: mixed-party ticket lifecycle을 보여주는 interactive Unicode TUI.
...
ticket 유입부터 match departure까지 직접 보려면 Sema Flow를 실행한다.

```sh
go run ./cmd/sema-tui
go run ./cmd/sema-tui -snapshot -steps 34
```
