...
P0 architecture foundation부터 P9 versioned durable service와 P10 operational validation까지 구현 중이다. deterministic composition부터 terminal assignment acknowledgment까지 실행 가능하며 single-writer journal이 reservation, assignment와 decision audit을 재시작 뒤 복구한다. `cmd/sema-ops-check`가 HTTP 부하와 restart/torn-tail recovery를 검증하지만 아직 production-ready deployment나 안정적인 SDK는 아니다.
